...
export default function EmailProcessingStatus({ 
  ...
  const getStatusBadge = () => {
    switch (status) {
      case 'waiting':
        return <Badge variant="outline" className="text-gray-600">Waiting</Badge>;
      case 'processing':
        return <Badge variant="secondary" className="bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-300">Processing</Badge>;
      case 'complete':
        return <Badge variant="secondary" className="bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300">Complete</Badge>;
      case 'error':
  ...
  return (
    <Card className={
      status === 'waiting' ? 'border-gray-200 dark:border-gray-700' :
      status === 'processing' ? 'border-blue-200 dark:border-blue-700' :
        ...
        <div className="space-y-2">
          ...
          {status === 'error' && (
            <div className="mt-2 text-sm text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20 p-2 rounded">
              Unable to process email. The format may not be supported or there might be missing information.
              <div className="mt-1">
                <span className="font-medium">Tip:</span> Try forwarding the original email without modifications.
              </div>
            </div>
          )}
        </div>
