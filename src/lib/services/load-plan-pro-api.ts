...
export interface FrontendOrder {
  ...
  status: 'pending_email_processing' | 'incomplete_dimensions' | 'processing' | 'complete';
...
export interface MissingItemsResponse { // Expected wrapper for fetchMissingOrderItems if backend sends it
  order_id: string; 
...
export interface BackendOrder { // Renamed from original Order interface
  id: string;
  ...
  status: 'pending' | 'loaded' | 'delivered'; // General order lifecycle status
  loading_instructions: {
    sequence: string[];
    notes: string;
    vehicleType: string;
    estimatedTime: string;
  } | any[]; // Match backend which can be empty list
}
 ...
 */
export const getCustomerOrders = (customerId: string): Promise<BackendOrder[]> => {
 ...
 */
export const updateOrderStatus = (orderId: string, status: 'pending' | 'loaded' | 'delivered'): Promise<BackendOrder> => {
  return authPut<BackendOrder>(`/customer/orders/${orderId}/status`, { status });
};
 ...
 */
export const createTestOrder = (): Promise<BackendOrder> => { // Assuming it returns BackendOrder structure
